...
This behavior can be compensated for by using a rosparameter: **deadzone** `[ROS docs]`_, which defines the amount by which the joystick has to move before it is considered to be off-center, specified relative to an axis normalized between -1 and 1.

.. _`[ROS DOCS]`: http://wiki.ros.org/joy#Parameters

Set the parameter in ``/etc/ros/DISTRO/robot.launch``:
